...
#define USE_PDI 0x08 //0x05-SPI;0x08-HBI“Ï≤Ω16
...
-----------------------------------------------*/
#if ECAT_TIMER_INT
    #define ECAT_TIMER_INC_P_MS                2000 //72000   	//0x271 /**< \brief 625 ticks per ms*/timer
#else
    #define ECAT_TIMER_INC_P_MS                168000   	//0x271 /**< \brief 625 ticks per ms*/ systick
#endif
...
-----------------------------------------------*/
    ...
    #define DISABLE_ESC_INT()            __disable_irq()	// {(_INT1IE)=0;} /**< \brief Disable interrupt source INT1*/
#endif

#ifndef ENABLE_ESC_INT
    #define ENABLE_ESC_INT()             __enable_irq()			// {(_INT1IE)=1;} /**< \brief Enable interrupt source INT1*/
#endif

#ifndef HW_GetTimer
    #if ECAT_TIMER_INT
        #define HW_GetTimer()       		 EtherCAT_TIMx->CNT					// ((UINT16)(TMR7)) /**< \brief Access to the hardware timer*/
    #else
        #define HW_GetTimer()               bsp_GetRunTime()
    #endif
#endif

#ifndef HW_ClearTimer
    #if ECAT_TIMER_INT
        #define HW_ClearTimer()              ((EtherCAT_TIMx->CNT)=0); //(SysTick->VAL = SysTick->LOAD)// //{(TMR7) = 0;} /**< \brief Clear the hardware timer*/
    #else
        #define HW_ClearTimer()              bsp_ClearRunTime();
    #endif
#endif
