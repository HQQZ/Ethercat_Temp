...
#endif
UINT8 DownloadType=0;/* ÏÂÔØÀàÐÍ0£ºF407,1:FPGA */
...
{
    uint8_t * pFileName = (uint8_t *)pName;

#if BOOTSTRAPMODE_SUPPORTED
    uint8_t * pMemName = (uint8_t *)aFirmwareDownloadHeader;
    uint8_t * pMemNameFPGA = (uint8_t *)aFPGADownloadHeader;
    if ( ( nameSize >= 4 )
            &&( pFileName[0] == pMemName[0] )
            &&( pFileName[1] == pMemName[1] )
            &&( pFileName[2] == pMemName[2] )
            &&( pFileName[3] == pMemName[3] )
       )
    {
        if ( bBootMode )
        {
            BL_StartDownload(0,password);
            DownloadType=0;
            return 0;
        }
        else
            return ECAT_FOE_ERRCODE_BOOTSTRAPONLY;
              ...
              &&( pFileName[0] == pMemNameFPGA[0] )
              &&( pFileName[1] == pMemNameFPGA[1] )
              &&( pFileName[2] == pMemNameFPGA[2] )
              &&( pFileName[3] == pMemNameFPGA[3] )
            )
    {
        if ( bBootMode )
        {
            BL_StartDownload(1,password);
            DownloadType=1;
            return 0;
        }
        else
...
UINT16 FOE_Data(UINT16 MBXMEM * pData, UINT16 Size)
{
    ...
    if ( bBootMode )
    {
        return BL_Data(DownloadType,pData, Size);
    }
    else
#endif
#if FOE_SAVE_FILES
