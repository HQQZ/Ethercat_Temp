...
void ECAT_SetLedIndication(void)
{
    ...
    static UINT8 system_error=0;


//	system_error=IsErrorHappend();

    // this code should be called every ms in average
    if ( bEcatOutputUpdateRunning && (system_error==0))// Run灯和Error灯复用，system_error=0表示没有发生错误
    {
...
//		debug2("nAlStatus=OP\r\n");
    ...
    else
    {
        ...
        if(ms == 50 || ms == 100 ||ms == 150 ||ms == 200)    //set flickering LED if required
        {
            ...
            if(system_error>0)
            {
                u8EcatRunLed = LED_DOUBLEFLASH;// 错误发送，双闪指示
//			    debug2("error haspend\r\n");
            }
            else
            {
...
{
//	GPIOB->BSRRL = GPIO_Pin_4;/* 测试中断执行时间*/

    if((nAlStatus & STATE_MASK)==STATE_BOOT)/*boot状态不刷新*/
    {
        return;
    }

    APPL_Application();/* 测试中断执行时间*/
